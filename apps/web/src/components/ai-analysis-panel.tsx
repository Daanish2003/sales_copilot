...
export default function AIAnalysisPanel({ analysis, isActive }: AIAnalysisPanelProps) {
  ...
  return (
    <div className="flex h-full flex-col gap-2 sm:gap-4">
        ...
        <div className="space-y-2">
          {analysis.concerns.length > 0 ? (
            analysis.concerns.map((concern, idx) => (
              <Badge key={idx} variant="secondary" className="block w-full text-center py-1 text-xs">
                {concern}
              </Badge>
            ))
          ) : (
      ...
      <Card className="flex flex-1 flex-col border border-border bg-card p-3 sm:p-4">
        <div className="mb-2 sm:mb-3 flex items-center gap-2">
          <Lightbulb className="h-4 w-4 text-yellow-500" />
          <p className="text-xs font-semibold text-muted-foreground">{t("suggestedResponse")}</p>
        </div>
        <div className="flex-1 rounded-lg bg-primary/10 border border-primary/30 p-2 sm:p-3 mb-2 sm:mb-3">
          {analysis.talkExample ? (
            <p className="text-xs sm:text-sm text-foreground leading-relaxed italic">"{analysis.talkExample}"</p>
          ) : (
